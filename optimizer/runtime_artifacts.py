"""Resolve production surrogate artifacts without silently falling back."""
...
DEFAULT_PRODUCTION_BUNDLE = Path("data/model-night-20260826-v2")
DEFAULT_PRODUCTION_MANIFEST = Path("data/surrogate-production.json")
...
def validate_runtime_economic_head(
    ...
    if getattr(head, "version", None) != artifacts.economic_model_version:
        raise RuntimeArtifactError(
            "economic head version differs from production manifest"
        )
...
def _feature_next_to_checkpoint(checkpoint: Path) -> Path:
    ...
    if adjacent.is_file():
        return adjacent
    bundle_level = checkpoint.parent.parent / "feature_context.json"
    return bundle_level
...
def resolve_runtime_artifacts(
    ...
    if explicit_checkpoint:
        ...
        context = (
            ...
            if explicit_context
            else _feature_next_to_checkpoint(checkpoint)
        )
        head = (
            Path(explicit_head) if explicit_head else checkpoint.parent / "npv_head.pt"
        )
        ...
        try:
            ...
            raise RuntimeArtifactError(
                f"invalid production manifest {manifest}: {error}"
            ) from error
        ...
        head = (
            Path(explicit_head)
            if explicit_head
            else manifest.parent / payload["npv_head"]
        )
        ...
        context = (
            Path(explicit_context)
            if explicit_context
            else bundle / "feature_context.json"
        )
        head = (
            Path(explicit_head)
            if explicit_head
            else bundle / "physical" / "npv_head.pt"
        )
        ...
        context = (
            Path(explicit_context)
            if explicit_context
            else directory / "feature_context.json"
        )
        head = Path(explicit_head) if explicit_head else directory / "npv_head.pt"
        source = "legacy AIOS_CHECKPOINT_DIR"
    elif DEFAULT_PRODUCTION_MANIFEST.is_file():
        manifest = DEFAULT_PRODUCTION_MANIFEST
        try:
            payload = json.loads(manifest.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError) as error:
            raise RuntimeArtifactError(
        ...
        bundle = DEFAULT_PRODUCTION_BUNDLE
        checkpoint = bundle / "physical" / "trajectory_ensemble.json"
