...
|---|---|---|---|
| Ранговый член в лоссе | да, параметр `ranking_loss_weight` в `Settings` (`backend/ml/surrogate/model.py`); при значении `> 0` включается попарный ранговый член | да, `ranking weight 3`, три пробных обучения (§11.2) | **нет: `ranking_loss_weight = 0`.** Обучение только Smooth L1 |
...
| Измеренный лаг λ в признаках | **нет.** `lag_months` есть в контракте `Lambda` (`backend/core/contracts/connectivity.py`), но `ScheduleFeatureizer` его не читает: в признаки идут только `matrix`, `window_start`, `window_end` (`backend/ml/surrogate/features.py`) | не проверялось: признака нет | **нет** |
| Рёбра λ на входе сети | структура `LambdaEdgeFeature` строится featureizer'ом | — | **нет: `lambda_edges` обнуляются** перед каждым вызовом модели — в поиске (`backend/application/optimization/schedule_search.py`), в тренере (`backend/ml/surrogate/train.py`), в проверке (`backend/presentation/cli/surrogate_check.py`) и в отчёте метрик |
