...
from contracts import ArtifactHashes, Budgets, NormativeSet

from config import COMPONENT_SEEDS, GLOBAL_SEED_KEY
from config.schema import DEFAULT_BUDGETS, default_seeds

OWNED_PACKAGES: tuple[str, ...] = ("connectivity", "policy", "robustness", "config")
...
def test_owned_packages_are_scanned() -> None:
    ...
    assert any(path.parts[-2] == "config" for path in sources)
...
def test_every_normative_field_is_reachable_from_the_config() -> None:
    from config.tests.conftest import a_hash

    from config import default_config
    from contracts import DEFAULT_NORMATIVES_2007
