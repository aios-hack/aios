...
from backend.application.jarvis.artifacts import RunError, RunRecord
from backend.application.jarvis.system_map import (
    SystemMap,
    SystemMapError,
    shared_system_map,
)
from backend.application.jarvis.tools.context import Card, ToolContext, ToolFailure
from backend.application.jarvis.tools.patterns import find_patterns
...
def _champion() -> dict[str, Any]:
    override = os.environ.get("AIOS_JARVIS_CHAMPION")
    path = Path(override) if override else _repository_root() / CHAMPION_FILE
    if not path.is_file():
        return {
            "recorded": False,
            "reason": (
