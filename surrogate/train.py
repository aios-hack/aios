...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default="absolute",
        help="watercut — контрактный набор целей (§5.1): нефть выводится из "
             "жидкости и обводнённости, а не предсказывается независимо. "
             "НЕ сочетать с --ranking-loss-weight: замерено Spearman -0.278 "
             "при ранге +0.930 на валидации, чемпион на последнем месте — "
             "ранговый лосс оптимизирует денежный прокси через обводнённость, "
             "минуя статьи, которых в прокси нет",
    )
    return parser
