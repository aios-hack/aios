...
function render(d){last=d;if(!selected||!d.stages.some(x=>x.id===selected))selected=d.active_stage;let s=d.stages.find(x=>x.id===selected)||d.stages[0],ds=s.dataset,tr=s.training||{history:[],metrics:null};$('tabs').innerHTML=d.stages.map(x=>`<button class="tab ${x.id===selected?'active':''}" data-id="${x.id}">${x.title} · ${x.status}</button>`).join('');document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{selected=b.dataset.id;render(last)});let p=ds.target?ds.completed/ds.target:0;
...
drawLoss(tr.history);$('trainingState').textContent=tr.phase_label||'Ожидает запуска';$('trainingState').className=`training-state ${tr.failed?'bad':tr.complete?'ok':'warn'}`;$('trainingDetails').innerHTML=metric('Фаза',tr.phase||'—')+metric('Эпоха',`${tr.current_epoch||0} / ${tr.max_epochs||'—'}`)+metric('Лучшая эпоха',tr.best_epoch||'—')+metric('Train loss',loss(tr.train_loss))+metric('Validation loss',loss(tr.validation_loss))+metric('Best validation',loss(tr.best_validation_loss));$('trainingError').textContent=tr.error||'';$('epoch').textContent=tr.history.length?`Эпоха ${tr.history.at(-1).epoch} · best ${tr.best_epoch||'—'}`:tr.phase_label||'Обучение ещё не началось';let m=tr.metrics;$('metrics').innerHTML='<div class="muted">Появятся после обучения и оценки.</div>';if(m){let rank=m.ranking||{},state=m.state_mean_per_scenario||{},ood=m.ood||{};$('metrics').innerHTML=metric('Spearman ЧДД',fmt(rank.spearman_rank_correlation))+metric('Precision@1',fmt((rank.precision_at_k||{})['1']))+metric('Regret@1',fmt((rank.regret_at_k_rub||{})['1'])+' ₽')+metric('ACTIVE/SHUT',pct(state.active_shut_accuracy||0))+metric('BHP MAE',fmt(state.bhp_mae_bar)+' bar')+metric('OOD outside',`${ood.n_scenarios_outside||0} / ${ood.n_scenarios||0}`)}}
