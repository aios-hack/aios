...
class WaterSupplyPolicy:
    """Полевой бюджет воды, извлечённый из ``Constraints.infrastructure``.

    ``reinjection_fraction=None`` означает, что отдельный баланс источника
    воды не задан. Число включает баланс: доступная закачка равна доле
    добытой воды плюс явно заданный внешний приток. ``lag_steps`` задаёт,
    с какого шага брать добытую воду; при лаге 1 первый шаг имеет только
    внешний приток.

    Три ключа оставлены в ``infrastructure`` намеренно: это уже существующая
    расширяемая часть JSON тестового кейса и редактора, поэтому новое
    уточнение организаторов не ломает старые документы Constraints.
    """
    ...
    def limit(self, produced_water_m3_per_day: float) -> float | None:
        if self.reinjection_fraction is None:
            return None
        return (
            self.external_water_m3_per_day
            + self.reinjection_fraction * max(0.0, produced_water_m3_per_day)
        )


@dataclass(frozen=True, slots=True)
class CompensationPolicy:
    """Целевой коридор компенсации, независимый от источника воды.

    Компенсация здесь совпадает с операционным определением R5: отношение
    закачанного объёма воды к отобранному объёму жидкости. ``diagnostic``
    означает цель оптимизатора; ``hard`` — ограничение тестового кейса.
    Даже hard-минимум не блокирует сдачу, если материальный баланс доказывает,
    что доступной воды для него физически недостаточно: это отдельная
    диагностика ``TARGET_UNREACHABLE``.
    """
...
def _finite_number(source: dict[str, object], key: str, default: float) -> float:
    ...
    if isinstance(raw, bool) or not isinstance(raw, (int, float)):
        raise ValueError(f"infrastructure.{key}: ожидается число, получено {raw!r}")
    value = float(raw)
    if not math.isfinite(value):
        raise ValueError(f"infrastructure.{key}: значение должно быть конечным")
    return value


def water_supply_policy(constraints: Constraints) -> WaterSupplyPolicy:
    """Разбирает и валидирует контракт доступной воды тестового кейса."""
    ...
    raw_lag: Any = source.get(WATER_REINJECTION_LAG_STEPS, 0)
    if isinstance(raw_lag, bool) or not isinstance(raw_lag, int) or raw_lag < 0:
        raise ValueError(
            f"infrastructure.{WATER_REINJECTION_LAG_STEPS}: ожидается целое "
            f"неотрицательное число шагов, получено {raw_lag!r}"
        )

    external = _finite_number(source, EXTERNAL_WATER_M3_PER_DAY, 0.0)
    if external < 0.0:
        raise ValueError(
            f"infrastructure.{EXTERNAL_WATER_M3_PER_DAY}: внешний приток не "
            f"может быть отрицательным, получено {external}"
        )
    if fraction is None and (
        WATER_REINJECTION_LAG_STEPS in source or EXTERNAL_WATER_M3_PER_DAY in source
    ):
        raise ValueError(
            f"infrastructure.{WATER_REINJECTION_FRACTION} обязателен, если "
            "задан лаг или внешний приток воды"
        )
    return WaterSupplyPolicy(
        reinjection_fraction=fraction,
        lag_steps=raw_lag,
        external_water_m3_per_day=external,
    )


def compensation_policy(constraints: Constraints) -> CompensationPolicy:
    """Разбирает коридор компенсации отдельно от материального баланса воды."""
    ...
    if minimum < 0.0:
        raise ValueError(
            f"infrastructure.{COMPENSATION_MIN}: нижняя граница отрицательна"
        )
    if maximum < minimum:
        raise ValueError(
            f"коридор компенсации пуст: {minimum}..{maximum}"
        )
