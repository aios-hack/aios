...
describe('FieldMap', () => {
  it('renders a point per well from the fixture', async () => {
    ...
    const ids = [...container.querySelectorAll('circle')].map((c) => c.getAttribute('data-well-id'));
  ...
  it('dims wells outside the selected layer and updates the counter', async () => {
    ...
    const active = container.querySelectorAll('circle[data-active="true"]');
    const dimmed = container.querySelectorAll('circle[data-active="false"]');
    ...
    expect(container.querySelectorAll('circle[data-active="true"]')).toHaveLength(2);
    fireEvent.click(screen.getByText(ru['map.layer.all']));
    expect(container.querySelectorAll('circle[data-active="true"]')).toHaveLength(3);
  });
