...
import {
  ...
  setVec2,
  setVec4Array
} from '@/jarvis/sphere/lib/sphereProgram';
import { emptyBolts, packBolts, stepBolts, type BoltField } from '@/jarvis/sphere/lib/sphereBolts';
import {
  SMOOTH_TAU_MS,
  approach,
  breathOfPhase,
  ...
  type SpherePalette,
  type SphereState
} from '@/jarvis/sphere/lib/sphereState';
...
export const useSphereRenderer = (
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const audioLevel = clamp01(current.audio);
      if (!still) {
      ...
      setFloat(gl, uniforms.u_burst, clamp01(current.burst));
