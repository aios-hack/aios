  ...
  return (
    <div className="jarvis-caption">
      <p
        ...
        aria-live="polite"
        aria-label={t('jarvis-screen.captionLabel')}
      ...
      {scene.warnings.map((warning) => (
        <p className="jarvis-caption-warning" key={warning.code}>
          <span className="jarvis-caption-warning-label">{t('jarvis-cards.warningLabel')}</span>
          {translateOr(t, `jarvis-screen.warning.${warning.code}`, 'jarvis-screen.warning.unknown')}
        </p>
      ))}
    </div>
  );
